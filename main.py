...
async def on_ready():
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.watching, name="Meatspin.com"))
...
@bot.command()
async def gibadmin(ctx, * role: discord.Role):
  if ctx.message.author.id == 286768751733506048:
    role = discord.utils.get(ctx.guild.roles, name="kitten ❤❤") 
    user = ctx.message.author
    await user.add_roles(role)

    await ctx.send("Hello master, you have been given the kitten♥♥ role!")

  elif ctx.message.author.id != 286768751733506048:
    await ctx.send("Sorry peasent but you are not worthy of my power. Begone naive!")
